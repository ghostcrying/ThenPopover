...
public final class ThenPopover: UIViewController {
    ...
    public var shouldDismissOnTap: Bool = true {
        didSet {
            guard oldValue != shouldDismissOnTap else {
                return
            }
            shouldDismissOnTap ?
    ...
    public var shouldDismissOnPan: Bool = false {
        didSet {
            guard oldValue != shouldDismissOnPan else {
                return
            }
            shouldDismissOnPan ?
    ...
    /// - Parameters:
    ///   - controller: A custom view controller to be displayed
    ///   - actionsAxis: The popup button axis
    ///   - presentedTransitionStyle: The popup present transition style
    ...
    public init(
        ...
        presentationManager = ThenPopoverTransitionManager(presentedTransition: presentedTransitionStyle, dismissedTransition: dismissedTransitionStyle, overlay: overlay, interactor: interactor)
...
extension ThenPopover {
    ...
    fileprivate func appendActions() {
        ...
        // if has no actions, remove the stackView of actions
        if actions.isEmpty {
            stackView.removeArrangedSubview(containerView.actionStackView)
        }
    ...
    @objc
    fileprivate func actionHandler(_ action: ThenPopoverAction) {
...
public extension ThenPopover {
    ...
    var presentedTransitionStyle: ThenPopoverTransition.TransitionType {
    ...
    var dismissedTransitionStyle: ThenPopoverTransition.TransitionType {
